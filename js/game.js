...
/*
   Action buttons belong to the answer box, but outside the answer-button
   container. They start hidden and appear after an answer is selected.
*/
function getActionArea(container) {
    if (!container) return null;

    const answerBox = container.closest(".answer-box") || container.parentElement;
    if (!answerBox) return null;

    let actionArea = qs(".answer-actions", answerBox);

    if (!actionArea) {
        actionArea = document.createElement("div");
        actionArea.className = "answer-actions";
        answerBox.appendChild(actionArea);
    }

    return actionArea;
}

function clearActionArea(container) {
    const actionArea = getActionArea(container);
    if (actionArea) actionArea.innerHTML = "";
    return actionArea;
}

function createSubmitButton(container, onSubmit) {
    const actionArea = clearActionArea(container);
...
function showSubmitButton(container) {
    const actionArea = getActionArea(container);
    const button = actionArea ? qs(".submit-button", actionArea) : null;
    show(button);
}

function createNextButton(container, text, onClick) {
    const actionArea = clearActionArea(container);
    if (!actionArea) return null;
...
function renderModeQuestion(mode, question, answers, onSelect, onSubmit) {
    ...
    createAnswerButtons(answersElement, answers, answer => {
        onSelect?.(answer);
        showSubmitButton(answersElement);
    });
